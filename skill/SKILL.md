...
- Fixed verified profiles: Claude `claude-opus-5-5/max`, Kimi `kimi-code/k3-256k/max`, OpenCode `deepseek/deepseek-flash/high`, Pi `openrouter/stealth/union-alpha/off`, Codex `gpt-6-sol/xhigh`. Never silently substitute a model or effort. Changing a profile requires adapting invocation and verification together. The Claude profile moved from `claude-opus-5` to `claude-opus-5-5` on 2026-09-23. Revising an older Claude job runs the new model on the currently resolved Claude Code CLI, because a job pins a versioned CLI that may not know the model, and refreshes an idle SDK connection first. Same-profile revisions keep the job's pinned CLI. The CLI must know `claude-opus-5-5`; 2.1.280 is the first version observed with it.
