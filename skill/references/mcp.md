...
| --- | --- | --- |
| `delegate_start` | owner, request_id, cwd, task, backend, permissions, inputs | Save a job and launch; Claude uses SDK, Kimi/OpenCode/Pi/Codex use CLI |
...
Kimi/OpenCode/Pi/Codex dispatch/status returns selected `tools`; Claude returns saved `allow_tools` rules. Kimi defaults include ReadMediaFile; Pi defaults to read/grep/find/ls; Codex `codex_tools` defaults to read (read-only sandbox). See [capabilities](tools.md). A previously loaded MCP may retain old defaults: inspect the receipt and use explicit selections or the updated CLI without redispatching an existing job.
