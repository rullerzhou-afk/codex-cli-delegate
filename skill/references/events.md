...
外层 `yield_time_ms` 要覆盖本轮剩余运行上限加一分钟；示例适配默认 30 分钟上限，用户明确设置更长时相应加长。若工具仍返回 `Script running with cell ID ...`，只用 `functions.wait` 继续同一 cell，并沿用足够长的外层等待时间，不另起 waiter。用户插话或取消等待时保留 job/cursor，先处理其指示。
...
10、15 分钟各触发一次，完成即取消。15 分钟以后，继续等错误或完成，原有默认 30 分钟硬上限仍有效；上限可在派单前用 `--timeout` 明确设置。不会无限每五分钟唤回一次。
