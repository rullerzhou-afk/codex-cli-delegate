...
CLI worker 按轮创建，SDK worker 可保留空闲会话；默认没有整轮硬超时，明确指定的数值时限仍生效。没有全局常驻服务。启动调用器的进程退出后，worker 可以继续本轮并留下结果，使重新打开的控制任务有记录可查。这不等于对所有 Codex 应用崩溃、系统重启、休眠或断网场景作出保证。
...
旧 job 仍保留原时限。检查原任务后可用 `revise --recover --timeout unlimited` 取消后续时限，继续原会话；未指定时沿用原值。
