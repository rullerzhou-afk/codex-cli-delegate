...
- **等待 review**：先审查已有结果，不重复原任务。补充已授权命令或参考资料直接 revise，程序处理空闲连接刷新。
- **已验收后追加修改**：直接 revise 同一 job；程序重新获取目录占用并保留旧验收记录。目录已被其他任务占用时，先协调实际冲突。
...
`claude-opus-5-5` 是模型，`max` 是思考深度，Max 是订阅名称。三者分别确认。每次调用带明确参数，实际模型从响应核对；实际深度从本次新增的 assistant 元数据核对，不查看或转述原始思考内容。
