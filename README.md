...
An established user alias for a backend counts as naming that route. A named external route is fulfilled only by a job on the matching external backend; a native Codex worker cannot satisfy it or be reported as that backend. If you explicitly name multiple external routes, each named route is dispatched. Those workers are requested participants, not extra reviewers added by default.
