...
没有明确的外部路由时，本 skill 不宣称路由优先级，也不新开外部任务；但它仍必须恢复并处置自己先前启动的外部任务。宿主路由 skill 可以选择原生 worker，本仓库既不观察也不控制该 worker。一旦本 skill 启动了外部任务，其 job／轮次／恢复／验收规则会一直生效到释放预订为止。若明确要求的外部路由不可用，应如实报告，绝不静默改用其他路由、模型或账号。参见[外部委派策略](skill/references/delegation-policy.md)。
...
当前统一测试结果为 **240 项 Python 和 11 项 JavaScript 全部通过**。使用 [统一测试入口](run_tests.py) 可输出一份机器可读汇总；进程身份测试需要读取系统进程信息，受限环境中的拒绝不能当成功。具体命令见[英文 README](README.md#tests)，不会调用付费模型。
