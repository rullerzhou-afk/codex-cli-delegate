...
用户已经建立的后端别名等同于点名对应的外部路由。点名的外部路由只有在匹配的外部后端 job 启动或续接后才算满足；原生 Codex worker 不能顶替或被汇报成该外部后端，也不能使用暗示该身份的名称、别名、译名或变体。若用户明确点名多个外部路由，应分别派发每条路线，并给每个 job 一项完整职责。多个只读审查者可以检查同一对象，但多个 worker 不能写入同一个 checkout。这些是用户点名的参与者，不属于默认追加外部审查者。
