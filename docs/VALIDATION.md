...
- **Current result:** 304 Python and 34 JavaScript tests pass across the
  ...
  process-title identity, and evidence-export fixtures. The Codex adapter adds
  JSON-stream, rollout, profile, sandbox, revision-baseline, and rate-limit
  fixtures. The Windows Kimi route adds runner, policy, packet, evidence
  reassembly, second-parse and revision fixtures. All fixture suites avoid
  paid model calls.
- **Split result:** `--exclude-process-identity` passed 219 Python plus 34
  JavaScript tests. `--only-process-identity`, run with permission to inspect
  real macOS child identities, passed 85 Python tests with no skips. Running
...
- **Policy text:** a documentation check keeps the delegation triggers and
  co-installation precedence present in the Skill and READMEs, and keeps the
  Codex reference's task-wording guidance, including the rule that confirmed
  security findings keep accurate terms. It is a text check, not a routing
  test, and it does not show how a provider classifies any wording.
...
## 3. Bounded Windows Codex and Kimi remote-exec evidence
...
- **Windows Kimi live runs (2026-09-23):** a Claude Code coordinator
  dispatched Kimi Code 0.42.0 (npm, Windows 11) over the same carrier with the
  fixed `kimi-code/k3-256k` / `max` profile. A read-only task, a same-session
  revision of it, and a write task in a scratch Git repository (Write and Bash)
  each passed the runner's first parse and the Mac's second Python parse of the
  fetched `state.json` and main `wire.jsonl`. Both parses covered the exact
  session and cwd, one run marker after the baseline, model and effort on every
  loop request, the bound tools, a clean ending, and the matching final text.
  The revision continued the same native session from the verified baseline
  and marked its parent superseded. An independent SSH read confirmed the
  written file and repository state. A read-only Codex regression run after the
  runner change also passed.
- **Windows Codex extra writable directory (2026-09-23):** a Claude Code
  coordinator dispatched a `workspace-write` Codex task with one policy-listed
  `--add-dir` for the Codex skills directory. The task updated the machine's
  own copy of this Skill from a Git checkout, after backing the old copy up to a
  temporary directory. Both the runner's first parse and the observer's second
  parse read `sandbox_policy` with `network_access` false and `writable_roots`
  exactly equal to the requested directory. An independent SSH comparison found
  the installed files identical to the checkout (66/66 SHA-256), a full backup,
  only stale `__pycache__` files removed, and the checkout itself unchanged.
- **Kimi answer quality is separate from evidence:** in the read-only run Kimi
  reported a wrong directory count and described a shell command it had no tool
  to run. The revision corrected the method but still missed one directory,
  probably because tool output was truncated. Evidence verification proves
  which model ran with which tools, not that the answer is right.
- **Does not prove (Kimi):** any sandbox (Kimi has none on Windows),
  carrier-loss recovery for Kimi, a live refusal of a session continued
  elsewhere (covered by fixtures only), long tasks, other Kimi versions, or web
  and MCP tools on Windows.
...
- A Linux or Windows port of the full local Claude/Kimi/OpenCode/Pi/Codex delegation
  and MCP/SDK lifecycle; the bounded Windows Codex/Kimi SSH path is narrower. GUI
